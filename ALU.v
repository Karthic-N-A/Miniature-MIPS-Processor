...
module ALU (
    ...
    always @(*) begin
        result = 32'b0; result_valid = 1'b0; branch_taken = 1'b0;
        branch_target = 8'b0;
        case (opcode)
            `OP_REG: begin
                case (func)
                    ...
                    `FUNC_JR:  begin 
                        branch_taken = 1'b1; 
                        branch_target = src1[7:0]; 
                    end
                    `FUNC_JALR: begin 
                        branch_taken = 1'b1; 
                        branch_target = src1[7:0]; 
                        result = {24'b0, pc + 8'd1}; 
                        result_valid = 1'b1; 
                    end
                    `FUNC_SLT: begin 
            ...
            `OP_ADDI: begin 
                result = src1 + {{16{imm[15]}}, imm}; 
                result_valid = 1'b1; 
            end
            `OP_ANDI: begin 
                result = src1 & {16'b0, imm}; 
                result_valid = 1'b1; 
            end
            `OP_ORI:  begin 
                result = src1 | {16'b0, imm}; 
            ...
            `OP_SLTI: begin 
                result = ($signed(src1) < $signed({{16{imm[15]}}, imm})) ? 32'd1 : 32'd0; 
                result_valid = 1'b1; 
            end
            `OP_SLTIU: begin 
                result = (src1 < {{16{imm[15]}}, imm}) ? 32'd1 : 32'd0; 
                result_valid = 1'b1; 
            end
            `OP_LUI:  begin 
                result = {imm, 16'b0}; 
            ...
            `OP_BLTZ_BGEZ: begin
                branch_target = pc + imm[7:0] + 1;
                if (rt == 1) branch_taken = ($signed(src1) >= 0) ? 1'b1 : 1'b0;
                else branch_taken = ($signed(src1) < 0) ? 1'b1 : 1'b0;
            end
            `OP_BEQ: begin 
                branch_target = pc + imm[7:0] + 1; 
